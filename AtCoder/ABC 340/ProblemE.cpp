...
void solve() {
    int boxNum, opNum; cin >> boxNum >> opNum;
    vi boxes;
    ...
    REP(i, 1, opNum) {
        int takenBox; cin >> takenBox;

        int mod = boxes[takenBox] % boxNum;
        int div = boxes[takenBox] / boxNum;
        boxes[takenBox] = 0;

        rep(i, boxNum)
        {
            if ((i > takenBox && i <= takenBox + mod)
                || (i < takenBox && i + boxNum <= takenBox + mod)
                )
            {
                boxes[i] += div + 1;
            }
